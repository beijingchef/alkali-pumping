...
def dipole_amplitude(I, Jg, Je, Fg, mg, Fe, me, q):
    """Return a real Wigner-convention hyperfine dipole amplitude.

    The common reduced electronic matrix element is omitted.  Squaring this
    amplitude reproduces :func:`dipole_strength`.  Retaining its sign is
    essential when a general polarization produces coherent Raman terms
    between different ground states.
    """
    if abs(me - mg - q) > 1e-9 or abs(q) > 1:
        return 0.0

    try:
        three_j = float(
            wigner_3j(S(Fe), S(1), S(Fg), S(-me), S(q), S(mg))
        )
        six_j = float(
            wigner_6j(S(Je), S(Fe), S(I), S(Fg), S(Jg), S(1))
        )
        phase_exponent = int(round(Fe - me + Je + I + Fg + 1.0))
    except Exception:
        return 0.0

    return float(
        (-1) ** phase_exponent
        * sqrt((2.0 * Fe + 1.0) * (2.0 * Fg + 1.0))
        * three_j
        * six_j
    )
